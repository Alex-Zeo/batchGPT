...
```bash
python interfaces/cli/main.py prompts/ --model gpt-4 --budget 10 --glob "*.md"
```

Read prompts from S3 and hide their contents in logs:
```bash
python interfaces/cli/main.py s3://my-bucket/prompts.txt --redact
```
...
```bash
python interfaces/cli/main.py myfile.pdf --model gpt-4o
```
...
```bash
python interfaces/cli/main.py path/to/file.pdf --model gpt-3.5-turbo --budget 5 --output results.jsonl
```
