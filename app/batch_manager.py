...
class BatchManager:
    ...
    def __init__(self, storage_dir: str | Path | None = None):
        if storage_dir is None:
            storage_dir = Path(__file__).parent / "batches"
    ...
    def get_all_batch_ids(self) -> List[str]:
        ...
        batch_files = glob.glob(str(self.storage_dir / "*.json"))
        for file_path in batch_files:
            batch_id = Path(file_path).name.replace(".json", "")
    ...
    def scan_batch_folder(self) -> None:
        ...
        if batches_dir.exists() and batches_dir.is_dir():
            ...
            batch_input_files = glob.glob(str(batches_dir / "batch_*.jsonl"))
            logger.info(
                f"Found {len(batch_input_files)} batch input files in {batches_dir}"
            )

            for input_file in batch_input_files:
                batch_id = Path(input_file).name.replace(".jsonl", "")
