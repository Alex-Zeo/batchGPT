...
async def list_batch_compatible_models() -> List[str]:
    ...
    try:
        ...
        test_file_path = BATCHES_DIR / f"batch_test_{uuid.uuid4().hex[:8]}.jsonl"
        ...
        test_request = {
            ...
            "body": {"messages": [{"role": "user", "content": test_content}]},
        }
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(
                    "https://api.openai.com/v1/batches/system", headers=headers
                ) as response:
                    ...
                    if response.status == 200:
                        ...
                        if "supported_models" in system_info:
                            logger.info(
                                f"Batch API reported supported models: {system_info['supported_models']}"
                            )
...
async def upload_batch_file(file_path: str) -> str:
    ...
    if not validate_api_key():
        raise ValueError(
            "Invalid or missing API key. Please check your OpenAI API key."
        )
...
async def create_batch_job(
    input_file_id: str, model: str, reasoning_effort: str
) -> Any:
    ...
    try:
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(
                "https://api.openai.com/v1/batches", json=data, headers=headers
            ) as response:
                if response.status != 200:
                    error_text = await response.text()
                    raise ValueError(
...
async def retrieve_batch_results(
    ...
    Returns:
        Tuple[List[EvaluationResult], List[Dict]]: The successful results and any error entries.
    ...
    if batch_status.status != "completed":
        logger.warning(
            f"Batch {batch_status.id} not completed yet. Status: {batch_status.status}"
        )
        return [], []
    ...
    try:
        ...
        logger.info(
            f"Retrieved {len(results)} successful results and {len(errors)} errors"
        )
...
async def prepare_batch_requests(
    ...
    Returns:
        List[BatchRequest]: Request dictionaries suitable for the batch API.
...
async def run_batch(
    ...
    if not validate_api_key():
        raise ValueError(
            "Invalid or missing API key. Please enter a valid OpenAI API key in the sidebar."
        )
    ...
    try:
        ...
        logger.info(f"Predicted cost for run: ${estimate['total']:.4f}")
    ...
    try:
        ...
        if model not in compatible_models:
            logger.warning(
                f"Model {model} may not be compatible with batch API. Using gpt-3.5-turbo instead."
            )
    ...
    if batch_size < total_requests:
        logger.info(
            f"Splitting {total_requests} requests into batches of size {batch_size}"
        )
        batch_jobs = []
        batch_errors = []
        ...
        if batch_errors:
            ...
            logger.warning(
                f"Some batches failed: {len(batch_errors)} errors, {len(batch_jobs)} successful"
            )

        return batch_jobs
    else:
        return await run_single_batch(
            inputs, model, reasoning_effort, temperature, max_tokens, response_format
        )
...
async def run_single_batch(
    ...
    try:
        ...
        input_path = BATCHES_DIR / temp_input_filename
        ...
        try:
            ...
            new_input_path = BATCHES_DIR / f"{batch_id}.jsonl"
            ...
            try:
                ...
                logger.warning(
                    f"Could not rename batch input file: {str(rename_error)}"
                )
            ...
            if "401" in error_message:
                ...
                raise ValueError(
                    "Rate limit exceeded. Please try again later or reduce batch size."
                ) from e
            elif "400" in error_message:
                if "model" in error_message.lower():
                    raise ValueError(
                        f"Invalid model: {model}. This model may not be available for batch processing."
                    ) from e
                else:
        ...
        try:
            ...
            if status.status != last_status and on_status_change:
                try:
                    ...
                    logger.error(
                        f"Error in status change callback: {str(callback_error)}"
                    )
            ...
            if status.status in ["completed", "failed", "expired", "cancelled"]:
                ...
                if status.status == "completed":
                    try:
                        ...
                        logger.error(
                            f"Error retrieving results for completed batch: {str(result_error)}"
                        )

                return status, results, errors

            logger.info(
                f"Batch {batch_id} status: {status.status}, waiting {poll_interval} seconds..."
            )
            ...
            if num_attempts >= max_retries:
                logger.error(
                    f"Failed to poll batch {batch_id} after {max_retries} attempts: {error_msg}"
                )
                break

            logger.warning(
                f"Error polling batch {batch_id} (attempt {num_attempts}/{max_retries}): {error_msg}"
            )
