...
def extract_text_from_pdf(file_content, extract_tables: bool = False) -> Tuple[str, Dict[str, Any]]:
    ...
    try:
                ...
                if extract_tables:
                    ...
                    if tables:
                        ...
                        for table_num, table in enumerate(tables):
                            if table:
                                table_text = "\nTable " + str(page_num + 1) + "-" + str(table_num + 1) + ":\n"
                                for row in table:
        ...
        logger.info(f"Successfully extracted {len(text)} characters from PDF with {metadata['pages']} pages")
...
def extract_text_from_code(file_content, extension) -> Tuple[str, Dict[str, Any]]:
    ...
    try:
        ...
        logger.info(f"Successfully extracted {len(text)} characters from {extension.upper()} file")
        return text, metadata
    except Exception as e:
...
def process_uploaded_file(file, file_type: Optional[str] = None, extract_tables: bool = False) -> Tuple[str, str, Dict[str, Any]]:
    ...
    if file_type is None:
        ...
        if file_name.endswith('.pdf'):
            ...
            file_type = 'py'
        elif file_name.endswith(('.js', '.html', '.css', '.json', '.ts', '.jsx', '.tsx')):
            file_type = 'code'
...
def process_multiple_files(files, extract_tables: bool = False, 
    ...
    if process_mode == "zip" and len(files) > 0:
        ...
        for file in files:
            if file.name.lower().endswith('.zip'):
                ...
                text, file_type, metadata = process_uploaded_file(file, extract_tables=extract_tables)
                zip_results.append({
                    "filename": file.name,
                    "text": text,
                    "file_type": file_type,
    ...
    if process_mode == "combine" and results:
        combined_text = "\n\n==== FILE: {} ====\n\n".format(results[0]["filename"]) + results[0]["text"]
        combined_metadata = {
            ...
            for root, _, files in os.walk(temp_dir):
                for filename in files:
                    ...
                    if file_size > 10 * 1024 * 1024:  # Skip files larger than 10MB
                        logger.warning(f"Skipping large file in ZIP: {rel_path} ({file_size} bytes)")
                        continue
            ...
            for rel_path, file_path in extracted_files:
                try:
                    ...
                    if ext == '.pdf':
                        ...
                        file_type = 'py'
                    elif ext in ('.js', '.html', '.css', '.json', '.ts', '.jsx', '.tsx'):
                        file_type = 'code'
                    else:
                        file_type = 'txt'
                    ...
                    with open(file_path, 'rb') as f:
                        ...
                        # Process the file
                        text, detected_type, metadata = process_uploaded_file(file_obj, file_type)
...
def generate_summary(processed_files: List[Dict]) -> str:
    ...
    for i, file in enumerate(processed_files, 1):
        ...
        if file_type == 'pdf':
            ...
            summary += f"   Pages: {pages}, Tables: {tables}, Contains Images: {has_images}\n"
        elif file_type == 'docx':
