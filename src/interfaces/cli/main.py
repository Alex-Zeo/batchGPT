...
@click.option("--output-format", type=click.Choice(["json", "csv", "xlsx"]), default="json")
def process_batch(directory: str, batch_size: int, output_format: str) -> None:
    """Dummy batch processing command with progress bar."""
    files = list(Path(directory).glob("*.pdf"))
    with click.progressbar(files, label="Processing") as bar:
        for _ in bar:
            pass
    click.echo(f"Processed {len(files)} files as {output_format} in batches of {batch_size}")
