...
from src.app.openai_batch import (
...
from src.utils.validators import is_valid_poll_interval, validate_api_key
...
from src.utils.parsers import deduplicate_prompts, expand_results
from src.app.file_processor import (
    process_uploaded_file, process_multiple_files, split_text_into_chunks,
    detect_file_type, generate_summary
)
from src.app.batch_manager import batch_manager
...
st.markdown("This application allows you to submit batch requests to OpenAI's API with advanced controls and monitoring features.")
...
with st.sidebar:
            ...
            with st.spinner("Fetching available models..."):
                try:
                    ...
                    if models:
                        ...
                        st.warning("No models found or API key may not have access to models")
                except Exception as e:
    ...
    default_model = "o1-pro" if "o1-pro" in st.session_state.available_models else (
        st.session_state.available_models[0] if st.session_state.available_models else "o1-pro"
    )
    
    model = st.selectbox(
        "Model", 
        options=st.session_state.available_models if st.session_state.available_models else ["o1-pro"],
    ...
    try:
        ...
        if not (0 <= temperature <= 2):
            st.sidebar.warning("Temperature should be between 0 and 2. Using default of 0.7.")
            temperature = 0.7
    except ValueError:
    ...
    try:
        ...
        if max_tokens <= 0:
            st.sidebar.warning("Max tokens should be a positive integer. Using default of 1024.")
            max_tokens = 1024
    except ValueError:
    ...
    try:
        ...
        if not (100 <= max_batch_size <= 10000):
            st.sidebar.warning("Batch size should be between 100 and 10000. Using default of 5000.")
            max_batch_size = 5000
    except ValueError:
    ...
    try:
        ...
        if max_chunk_size < 100:
            st.sidebar.warning("Chunk size should be at least 100. Using default of 4000.")
            max_chunk_size = 4000
    except ValueError:
    ...
    try:
        ...
        if chunk_overlap < 0:
            st.sidebar.warning("Chunk overlap should be non-negative. Using default of 200.")
            chunk_overlap = 200
    except ValueError:
...
tab1, tab2, tab3, tab4, tab5 = st.tabs(["Submit Batch", "Monitor Batches", "Results", "Statistics", "Logs"])
...
with tab1:
    ...
    if not st.session_state.api_key_valid:
        st.error("⚠️ Please enter a valid OpenAI API key in the sidebar before submitting batches.")
        ...
        if process_button:
            with st.spinner(f"Processing {len(uploaded_files)} files..."):
                ...
                if processed_files:
                    ...
                    use_as_prompts = st.checkbox("Use processed files as prompts", value=True)
                            ...
                            for file in processed_files:
                                ...
                                if len(file["text"]) > max_chunk_size:
                                    ...
                                    for i, chunk in enumerate(chunks):
                                        prompts.append(f"[{file['filename']} - Part {i+1}/{len(chunks)}] {chunk}")
                                else:
                                    prompts.append(f"[{file['filename']}] {file['text']}")
    ...
    if st.session_state.processed_files:
        ...
        file_tabs = st.tabs([file["filename"] for file in st.session_state.processed_files[:10]])
        ...
        for i, tab in enumerate(file_tabs):
            with tab:
                ...
                metadata_html = "<table style='width: 100%; border-collapse: collapse;'>"
            ...
            if len(prompts) == 0:
                ...
                with st.spinner(f"Submitting batch of {len(prompts)} prompts..."):
                    try:
                        ...
                        batch_job = asyncio.run(run_batch(
                            inputs=prompts,
                            model=model,
                            reasoning_effort=reasoning_effort,
                            temperature=temperature,
                        ...
                        if isinstance(batch_job, list):
                            ...
                            status_text.success(f"Submitted {len(batch_job)} batches successfully!")
                        ...
                        if "401" in str(e) or "authentication" in str(e).lower() or "api key" in str(e).lower():
                            st.info("This appears to be an authentication issue. Please check your API key.")
...
with tab2:
    ...
    # Show API key warning if needed
    if not st.session_state.api_key_valid:
        st.error("⚠️ Please enter a valid OpenAI API key in the sidebar to monitor batches.")
    ...
    with col2:
        ...
        if st.session_state.polling_active:
            st.info("🔄 Auto-polling is active. Checking batch status every " + 
                   f"{poll_interval_seconds} seconds.")
        
    with col3:
        # Allow refreshing a specific batch
        in_progress_batches = [bid for bid, batch in st.session_state.batch_jobs.items() 
            ...
            if st.button("Refresh Selected Batch"):
                with st.spinner(f"Refreshing status of batch {selected_batch}..."):
                    try:
                        ...
                        # Get updated status
                        batch_status = asyncio.run(retrieve_batch_status(selected_batch))
                        ...
                        if batch_status.status == "completed":
                            ...
                            st.success(f"Batch {selected_batch} is completed with {len(results)} results and {len(errors)} errors")
                        else:
                            # Update status only
                            batch_manager.update_batch(
                                selected_batch, 
                            ...
                            st.info(f"Batch {selected_batch} status: {batch_status.status}")
                        
                        # Update session state
                        st.session_state.batch_jobs[selected_batch] = batch_manager.get_batch(selected_batch)
                    except Exception as e:
    ...
    with col1:
        batch_status_id = st.text_input("Add Batch ID", placeholder="Enter a batch ID to monitor")
    with col2:
        add_batch_disabled = not st.session_state.api_key_valid
        if st.button("Add Batch", disabled=add_batch_disabled) and batch_status_id.strip():
            if batch_status_id in st.session_state.batch_jobs:
                ...
                with st.spinner(f"Getting info for batch {batch_status_id}..."):
                    try:
                        ...
                        batch_status = asyncio.run(retrieve_batch_status(batch_status_id.strip()))
                        batch_info = {
                            "id": batch_status_id,
                            "status": batch_status.status,
                            "created_at": batch_status.created_at,
                            "model": batch_status.model if hasattr(batch_status, 'model') else "Unknown",
                        ...
                        if "404" in str(e) or "not found" in str(e).lower():
                            st.warning(f"Batch ID '{batch_status_id}' not found. Please check the ID.")
                        elif "401" in str(e) or "authentication" in str(e).lower() or "api key" in str(e).lower():
                            st.info("This appears to be an authentication issue. Please check your API key.")
    ...
    if st.session_state.batch_jobs:
        ...
        with filter_col2:
            model_filter = st.multiselect(
                "Filter by Model",
                options=set(batch.get("model", "Unknown") for batch in st.session_state.batch_jobs.values() if batch),
                default=[]
            )
    ...
    if st.session_state.polling_active and st.session_state.api_key_valid:
        ...
        in_progress_batch_ids = [
            bid for bid, batch in st.session_state.batch_jobs.items()
            if batch and batch.get("status") in ["in_progress", "validating"]
        ]
        ...
        if in_progress_batch_ids:
            ...
            try:
                ...
                if batch_status.status == "completed":
                    ...
                    logger.info(f"Batch {batch_to_update} completed with {len(results)} results")
                else:
                    # Just update status
                    batch_manager.update_batch(
                        batch_to_update,
                    ...
                    logger.info(f"Updated batch {batch_to_update} status: {batch_status.status}")
                
                # Update session state
                st.session_state.batch_jobs[batch_to_update] = batch_manager.get_batch(batch_to_update)
                
                # Rerun the app after poll_interval_seconds
                time.sleep(poll_interval_seconds / 2)  # Sleep for half the interval to avoid too frequent reruns
                st.rerun()
            except Exception as e:
    ...
    if batch_ids:
        ...
        if batch_info.get("results"):
            ...
            if len(df) > 0:
                ...
                st.info(f"Estimated cost: ${cost['total']:.4f} (Input: ${cost['input']:.4f}, Output: ${cost['output']:.4f})")
                
                # Token usage visualization
                fig = px.bar(
                    pd.DataFrame({
                        'Token Type': ['Input', 'Output', 'Reasoning'],
                        'Count': [df["input_tokens"].sum(), df["output_tokens"].sum(), df["reasoning_tokens"].sum()]
                    }),
                    x='Token Type',
                    y='Count',
            ...
            with st.expander("View Results Table", expanded=True):
                st.dataframe(df[["custom_id", "content", "input_tokens", "output_tokens", "reasoning_tokens", "total_tokens"]], use_container_width=True)
        ...
        if batch_info.get("errors"):
            ...
            for error in batch_info["errors"]:
                error_data.append({
                    ...
                    "Error Message": error.get("error", {}).get("message", "Unknown")
                })
...
with tab4:
    ...
    if batch_stats["completed"] > 0:
        ...
        if sum(token_data.values()) > 0:
            ...
            model_df = pd.DataFrame({
                'model': list(batch_models.keys()),
                'input_tokens': [token_data['Input'] * (batch_models[m] / total_requests) for m in batch_models.keys()],
                'output_tokens': [token_data['Output'] * (batch_models[m] / total_requests) for m in batch_models.keys()],
                'reasoning_tokens': [token_data['Reasoning'] * (batch_models[m] / total_requests) for m in batch_models.keys()]
            })
            ...
            st.info("💡 Pro tip: This is an estimated cost based on current pricing. Actual billing may vary.")
    ...
    with col1:
        # Prune old batches
        prune_days = st.number_input("Prune batches older than (days)", min_value=1, value=30)
...
# Footer
st.markdown("---")
st.markdown("**OpenAI o1-Pro Batch API Interface** | Built with Streamlit", unsafe_allow_html=True)
